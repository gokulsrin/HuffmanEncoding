...
public class BinaryTree<E> {
	...
	public BinaryTree<E> copyToDepth(int d){
		...
		if(d > 0){
			if(hasLeft() && hasRight()){
				return new BinaryTree<>(data, left.copyToDepth(d -1), right.copyToDepth(d -1));
			}
			else if(hasRight() && !hasLeft()){
				return new BinaryTree<>(data, right.copyToDepth(d - 1), null);
			}
			else if(hasLeft() && !hasRight()){
				return new BinaryTree<>(data, null, left.copyToDepth(d - 1));
			}
			else{
				return new BinaryTree<>(data);
			}
